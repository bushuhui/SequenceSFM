...
int convIpoint2KeyPoint(IpVec &ipts, std::vector<cv::KeyPoint> &kps)
{
    IpVec::iterator             it1;
    int                         i;
    cv::KeyPoint                *p;

    kps.resize(ipts.size());

    for(it1=ipts.begin(), i=0; it1!=ipts.end(); it1++, i++) {
        p = &(kps[i]);

        p->pt.x = it1->x;
        p->pt.y = it1->y;

        p->angle = it1->orientation;
...
int getMatches(IpVec &ipts1, IpVec &ipts2, std::vector<cv::DMatch> &matches, double th)
{
    float   dist, d1, d2;
    Ipoint  *match = NULL;
